...
<template>
  <div class="space-y-2 -mt-2 mb-2">
    ...
    <template v-else-if="activeItems.length > 0">
      <JointGiftCard v-for="(gift, index) in activeItems" :key="gift.id" :gift="gift"
    ...
    <div v-else-if="!jointGiftStore.isLoading && activeItems.length === 0 && archivedItems.length === 0"
      ...
      <UButton v-if="pairStore.isHost" color="primary" variant="subtle" leading-icon="i-lucide-plus"
    ...
    <UCollapsible v-if="archivedItems.length > 0" class="group animate-slide-up opacity-0 translate-y-5 mb-4" :style="`animation-delay: ${0.2 + activeItems.length * 0.05}s`">
      ...
      <template #content>
        <div class="space-y-2 mt-2">
          <JointGiftCard v-for="(gift, index) in archivedItems" :key="gift.id" :gift="gift" class="opacity-60"
